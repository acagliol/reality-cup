import { SPONSOR_AI_MODELS } from '../ai/sponsorModels';
...
export async function fetchPlatformInsights(): Promise<PlatformInsights> {
  ...
  const enriched = roundList.map((row) => {
    ...
    return { row, aiMean, aiSpread, aiValues, crowd };
  });
  ...
  const sortAscHardest = (items: InsightPhoto[]) =>
    [...items].sort((a, b) => b.score - a.score).slice(0, 6);
  ...
  return {
    ...
    hardestFakes: sortAscHardest(hardestFakes),
